...
cc._RF.push(module, '71936Mgb/BF5b1ySWk+SFXw', 'gameScene', __filename);
// Script/gameScene.js
...
cc.Class({
    ...
    //增加身体长度奖励
    onGotBodyReward: function onGotBodyReward(count) {
    ...
    onTouchCallBack: function onTouchCallBack() {
        this.touchFlag++;
        if (this.touchFlag == 2) {
    ...
    moveHero: function moveHero(self) {
        //开始点击
    ...
    onSetPosition: function onSetPosition(pos, isRightToLeft) {
    ...
    onSetHeroPosition: function onSetHeroPosition(num) {
        this.Hero.setPositionX(this.Hero.getPositionX() + num);
    },

    onGetHeroPosition: function onGetHeroPosition() {
    ...
    /*****************body start******************/
    //在最后一个位置添加count节长度
    onAddBodyToLast: function onAddBodyToLast(count) {
    ...
    //添加body节点到场景
    onAddBodyFromPool: function onAddBodyFromPool(index) {
        var body = null;
    ...
    lateUpdate: function lateUpdate(dt) {
        if (this.Hero.getPositionX() > cc.director.getWinSize().width / 2 - this.Hero.width / 2 || this.Hero.getPositionX() < -cc.director.getWinSize().width / 2 + this.Hero.width / 2) {
            //改变蛇头的角度
            this.Hero.getComponent('hero').onRecoveryRotation();
        }
